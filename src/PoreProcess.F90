...
!          unit=1,   file='output.txt'
!          unit=2,   file='particleposition'
!          unit=3,   file='solidposition.txt'
!          unit=4,   file='operateconditions.txt'
!          unit=5,   file='MultiFluid.txt'
!          unit=6,   file='Solid.txt'
!          unit=7,   file='isothermdata.txt'  
!          unit=8,   file='Bojan.txt'
!          unit=9,   file='PressureGCMC.txt'
!          unit=10,  file='firstsurface.txt'
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
      write(*,*) '----------------------------------------'
      write(*,*) 'ENTER FILENAME TO STORE OUTPUT DETAILS ='
      write(*,*) '----------------------------------------'
!      read(*,*) filename
      ...
      write(*,*) '------------------------------------------------'
      write(*,*) 'ENTER FILENAME TO STORE POSITION OF PARTICLES = '
      write(*,*) '------------------------------------------------'
!      read(*,*) filename
      open( unit=2, file='particleposition.txt', status='unknown')
      rewind(2)
      
      open( unit=3, file='solidposition.txt', status='unknown')
      rewind(3)
      
      
      open( unit=7, file='isothermdata.txt', status='unknown')
      rewind(7)
      write(7,*)'-----------------------------------------------------------------------------------------------------------------------------------------------------------------'
      write(7,'(A4,18A14)') 'no.','rCutoff','P','P/p0','SurfE','Nb_rho','Ex_Nb_rho','Iso_H','Iso_HFF','Iso_HSF','nIso_H','nIso_HFF','nIso_HSF','nnIso_H','Av_Npart','Av_Ninpore','Ad_Ninpore','CoorN-1','CoorN-2' 
      write(7,'(5x,6A14)')'(-)','(pa)','(-)','(umol/m2)','(mol/m3)','(mol/m3)','(kJ/mol)'
      write(7,*)'-----------------------------------------------------------------------------------------------------------------------------------------------------------------'
...
!      ----------------------------     
        open(unit=5, file = 'MultiFluid.txt',status='unknown')
        rewind(5)
        read(5,*)MW   ! MOLECULAR WEIGHT
        read(5,'(f6.2)')CriticalT
        read(5,*)MCx0, MCy0, MCz0
        read(5,*)NLJ
        do j = 1, NLJ
           read(5,*)LJx0(j), LJy0(j),LJz0(j)
           read(5,*)sigmaFF(j),wellDepthFF(j)
        enddo
        read(5,'(I)')NCL
        do k = 1, NCL
           read(5,*)CLx0(k), CLy0(k), CLz0(k)
           read(5,*)Charges(k)
       ...
       open(unit=4, file='operateconditions.txt')
!      --------------------------- 
...
!      ---------------------------
       ...
       read(4,*)WAGNER
       read(4,'(L)')SDisplace 
       read(4,'(L)')HorizontalBin
       read(4,'(L)')VerticalBin
       read(4,'(L)')TradMC
       read(4,'(L)')ExtraBin
       read(4,'(L)')ChangeXY
       read(4,'(L)')DistanceD
...
!     ----------------------------------------    
       open(unit=10, file='firstsurface.txt')
       read(10,*)CarbonLengthx1         !! IF GCMC, this is for the bulk phase
       read(10,*)CarbonLengthy1
       read(10,*)BoxLengthZ
...
!       ---------------------------------------------------------------
        IF(INPUTCP)THEN
             ...
             read(13,*)NumOfPre
         ELSE
              open(unit=9,file='PressureGCMC.txt',status='unknown')
              rewind(9)
              read(9,*)NumOfPre
         ENDIF
         ...
         ELSE
              ...
              do iP = 1,NumOfPre
                 ...
                 write(1,*)'iP,ChemicalPotential,ChemicalPotentialJoulepermol',iP,ChemicalPotential(iP),ChemicalPotentialJoulepermol(iP)
!                ZV = (ChemicalPotential/Temperature) + log(deBro)
         ...
         read(10,*)CarbonLengthx1  !! Simulation box for adsorption in GCMC
         read(10,*)CarbonLengthy1
         read(10,*)BoxLengthZ
         read(10,*)AccVYL
         read(10,*)AccVYH
...
!      ------------------------------
 ...
 !      IF(Close2Ends)Close1End = .FALSE.
          ...
          if(Bojan)then
            open(unit=8, file='Bojan.txt')
            read(8,*)BojanSlit
            read(8,*)BoPBCy
            read(8,*)NS
            do i = 1, NS
              read(8,*)TopStrip(i)  ! If the strip is on the top of the simulation box or not
              read(8,*)StripZ0(i)
              read(8,*)StripC(i)
              read(8,*)StripW(i)
              read(8,*)StriplayerN(i)
              read(8,*)Stripgap(i)
              read(8,*)BsigmaSS(i)
              read(8,*)BwelldepthSS(i)
              read(8,*)rhosperM2(i)
              read(8,*)StripAngleY(i)
              read(8,*)
            enddo
         endif
         if(Bojan)then
       ...
       read(4,'(L)')Sep_rho_Dis
       read(4,'(L)')CorCompress
       if(Sep_rho_Dis)then
       ...
       read(4,'(L)')SMediation1
       read(4,*)ksi
       read(4,'(L)')SMediation2
       read(4,*)SMLimitZ
       read(4,*)SMLimitZ2
       read(4,*)ksiF
       SMLimitZ = SMLimitZ/SCALELENGTH
       SMLimitZ2 = SMLimitZ2/SCALELENGTH
       read(4,'(L)')FlexiCL     !If the location of charges is flexiable or not
       if(FlexiCL)then
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++     
      ...
      write(*,1119) T,  Ncycle,  Ncycles, Nmove
      write(1,1119) T,  Ncycle,  Ncycles, Nmove
...
1120        FORMAT(1x,'The number of LJ site----------------------: ', i10)
       do i = 1, NLJ
       write(*,1121) i,sigmaFF(i),welldepthFF(i)
...
1122        FORMAT(1x,'The number of CL site---------------------: ', i10)
       do i = 1, NCL
       write(*,1123)i, Charges(i)
1123        FORMAT(1X,'NCL=',I5, 5x,'Charges = ', e15.7)        
       enddo
       

       write(*,1124) sigmaSS, WellDepthSS
       write(1,1124) sigmaSS, WellDepthSS
...
!    ------------------------------------------------------
     ...
     if(rcutoff.gt.rcutoffset)then
       write(*,*)'---------------------------------------- '
       write(*,*)'Which one would be chosen as the rcutoff:'
       write(*,*)'---------------------------------------- '
       write(*,*)'1-', rcutoff
       write(*,*)'2-', rcutoffset
!       read(*,*) cutoffopt
...
!    --------------------------------
   ...
   !      endif
     ...
     write(*,*)'------------------------------------- '
     write(*,*)'Calculating the Access Volume:        '
     write(*,*)'------------------------------------- '  
...
!     AccVolume = (BoxLengthZ-SCALELENGTH)*CarbonLengthx1*CarbonLengthy1
     ...
     write(7,*)'AccVolumeM3',AccVolumeM3
     
     write(*,1130) TotSurfaceArea, BulkVolume, AccVolume
     write(1,1130) TotSurfaceArea, BulkVolume, AccVolume
1130 format( 1x, 'SurfaceArea(-)--------------------------:', f20.10,/, &
  &          1x, 'BulkVolume(-)---------------------------:', f20.10,/, &
  &          1x, 'AccVolume(-)----------------------------:', f20.10,/)  
     
     if(inclinepore.OR. SurfPore)then
     write(*,1141) SurfaceArea1, SurfaceArea2,TotSurfaceArea,BulkVolume, AccVolume
     write(1,1141) SurfaceArea1, SurfaceArea2,TotSurfaceArea,BulkVolume, AccVolume
