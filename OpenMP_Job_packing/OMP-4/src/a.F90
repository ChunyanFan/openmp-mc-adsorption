...
!       call fluctuation(0)
    ...
    do iPC = 1 , NumOfPre
       write(62,*)'-----------------------------------------------------------------'
       write(62,*)'The pressure being calculated is _ of _ - _:', iPC,NumOfPre,P(iPC)
       write(62,*)'-----------------------------------------------------------------'    
!      ------------
!      START TIMING
!      ------------
...
!      -------------------------
       ...
       elseif(TradMC)then
          ...
          deltaRz   = deltaRzInitial
           write(62,*)'deltaRx,deltaRy,deltaRz',deltaRx,deltaRy,deltaRz
       endif
!      --------------------------------
!      INITIAL POSITION ALL N PARTICLES
!      --------------------------------
!       call Initialization
        write(51,*)P(iPC),rho(iPC)
!      -------------------------------- 
       ...
       IF(SDisplace.AND.(.NOT. TradMC))THEN
         ...
         do iEqui = 1, Ncycle1
             ...
             if (mod(iEqui,1000).eq.0)then
                write(62,*)'==============Mu-GCMC EQULIBRATION==================='
                write(51,*)'==============Mu-GCMC EQULIBRATION==================='
                write(62,646)iEqui, sum_Npart/numberofequilibrium, EnergyTotal/numberofequilibrium
                write(51,646)iEqui, sum_Npart/numberofequilibrium, EnergyTotal/numberofequilibrium
...
279      FORMAT(1x,E11.4,45E18.8)
       ...
       ELSEIF(TradMC)THEN    !Traditional GCMC
             ...
             do iEqui = 1, Ncycle
             if (mod(iEqui,1000).eq.0)then
                write(62,*)'================GCMC EQULIBRATION==================='
                write(51,*)'================GCMC EQULIBRATION==================='
                write(62,645)iEqui, sum_Npart/numberofequilibrium, EnergyTotal/numberofequilibrium
                write(51,645)iEqui, sum_Npart/numberofequilibrium, EnergyTotal/numberofequilibrium
      645       format(1x,'CYCLES:',I8,'  Npart:',e15.6,'  AveEnergy:',E15.6)
                write(62,649)P(iPC),totAcceptanceRatio/dble(iEqui),deltaRx
                write(62,650)deltaRy,deltaRz
                write(51,649)P(iPC),totAcceptanceRatio/dble(iEqui),deltaRx
                write(51,650)deltaRy,deltaRz
...
341          FORMAT(1x,E11.4,3E18.8)
     ...
     ENDIF
     write(62,*) '1284 TEnergy, Energy, CLEnergy, EnergyC', TEnergy, Energy, CLEnergy, EnergyC
!    -----------------------------
    ...
    ENDIF
     ...
     IF(SDisplace.AND.(.NOT. TradMC))THEN
         ...
         do iCycle=1, NCycles1
             ...
             if (mod(iCycle,1000).eq.0)then
                write(62,*)'===============Mu-GCMC SAMPLING==================='
                write(51,*)'===============Mu-GCMC SAMPLING==================='
                write(62,726)iCycle, sum_Npart/numberOfSampling, EnergyTotal/numberOfSampling
                write(51,726)iCycle, sum_Npart/numberOfSampling, EnergyTotal/numberOfSampling
     ...
     ELSEIF(TradMC)THEN
        do iCycle=1, NCycles
            ...
            if( mod(iCycle,1000).eq.0) then
               write(62,*)'==================GCMC SAMPLING====================='
               write(51,*)'==================GCMC SAMPLING====================='
               write(62,716)iCycle, sum_Npart/numberOfSampling, EnergyTotal/numberOfSampling
               write(51,716)iCycle, sum_Npart/numberOfSampling, EnergyTotal/numberOfSampling
      716      format(1x,'CYCLES:',I8,'  Npart:',e15.6,'  AveEnergy:',e15.6)
               write(62,719)P(iPC),totAcceptanceRatio/dble(icycle),deltaRx
               write(62,720)deltaRy,deltaRz
               write(51,719)P(iPC),totAcceptanceRatio/dble(icycle),deltaRx
               write(51,720)deltaRy,deltaRz
...
!    ------------------------
     ...
     IF(Iso_NN .ne. Iso_Npart*Iso_Npart )then
        ...
        write(62,*)'Iso_NN = Iso_Npart*Iso_Npart'   
        write(51,*)'Iso_NN = Iso_Npart*Iso_Npart' 
     endif  
...
!    ------------------
     ...
     write(62,30)  rho(iPC),Ns_rho,Nb_rho,Ex_Nb_rho, AccVolume, Av_Npart,Av_Ninpore,Ad_Ninpore, &
...
!                 Nb_rho, Nb_rhoMolPerM3               
     write(51,30)  rho(iPC),Ns_rho,Nb_rho,Ex_Nb_rho, AccVolume,Av_Npart,Av_Ninpore,Ad_Ninpore, &
       &          Pressure(iPC),  EnergyAverage,Iso_Heat, &
       &          Iso_HeatFF, Iso_HeatSF, &
       &          NIso_Heat, NIso_HeatFF, NIso_HeatSF, &
       &          Ns_rhoUMolPerM2,Nb_rhoMolPerM3,Ex_Nb_rhoMolPerM3,P(iPC), AccVolumeM3, &
...
!      &      1x, 'LJ-Porewidth-H (-) ------------------------- : ', f20.10,/, &
       ...
       &      1x, 'Time (sec) --------------------------------- : ', f20.10/)
       write(51,*)'average of Total Energy(-)',Iso_TEnergy
!              -------------------------------
!              STORE THE POSITION OF PARTICLES
!              -------------------------------
               write(52,*) 'Npart=', Npart
               do i = 1, Npart
                  write(52,435) P(iPC),mcx(i), mcy(i), mcz(i) !, Lequi(i)*scalelength
435               format(5f16.7)
               enddo  
               write(52,*) 'LJSITES' 
               do i = 1, Npart
                  do j = 1, NLJ
                    write(52,435)P(iPC), LJX(i,j),LJY(i,j),LJZ(i,j)
                  enddo
               enddo
               write(52,*) 'COULOMBSITES' 
               do i = 1, Npart
                  do j = 1, NCL
                    write(52,435)P(iPC), CLX(i,j),CLY(i,j),CLZ(i,j)
                  enddo
               enddo
...
!    ----------------------------------
        write(57,712)iPC,rCutoff,P(iPC),P(iPC)/Psaturate,Ns_rhoUMolPerM2,Nb_rhoMolPerM3,Ex_Nb_rhoMolPerM3,Iso_HeatKJoulePerMol,Iso_HeatFFKJoulePerMol, Iso_HeatSFKJoulePerMol, &
...
!    finalLD = .TRUE.
   ...
   DO ibranch =1, Nbranch
       do iden = 1 , Numofrho
        write(62,*)'----------------------------NVT ENSEMBLE-------------------------------'
        write(62,*)'The point being calculated is _ of _ ,Npart:', iden,Numofrho,Npart+addN 
        write(62,*)'-----------------------------------------------------------------------'    
!       ------------
!       START TIMING
!       ------------
...
!       -----------------
        ...
        IF(SDisplace .AND. (.NOT. TradMC))THEN
            ...
            do iEqui = 1, Ncycle1
               ...
               if (mod(iEqui,1000).eq.0)then
                  write(62,*)'=============Mu-NVT EQULIBRATION==================='
                  write(51,*)'=============Mu-NVT EQULIBRATION==================='
                  write(62,646)iEqui, EnergyTotal/numberofequilibrium/dble(Npart)
                  write(51,646)iEqui, EnergyTotal/numberofequilibrium/dble(Npart)
      646         format(1x,'CYCLES:',I8, '  AveEnergy:',3E15.6)
                  write(62,648)Npart,totAcceptanceRatio/dble(iEqui),deltaRx
                  write(51,648)Npart,totAcceptanceRatio/dble(iEqui),deltaRx
...
552                          FORMAT(1x,I5,I12,30E14.4) 
            ...
            IF(ChangeXY)THEN  !AT THE END OF EQULIBRIM STAGE RESET THE BOX WITH THE AVERAGE BOX LENGTH
               CALL BOXRESET(SumCarbonlengthx1,SumCarbonlengthy1,SumBoxlengthZ,NChange)
            ENDIF
            write(62,*) '1805 TEnergy, Energy, CLEnergy, EnergyC', TEnergy, Energy, CLEnergy, EnergyC
...
553         FORMAT(1x,I4,45E18.8)
        ...
        ELSEIF(TradMC)THEN
            ...
            do iEqui = 1, Ncycle
               ...
               if (mod(iEqui,1000).eq.0)then
                  write(62,*)'================NVT EQULIBRATION==================='
                  write(51,*)'================NVT EQULIBRATION==================='
                  write(62,642)iEqui, EnergyTotal/numberofequilibrium/dble(Npart)
                  write(51,642)iEqui, EnergyTotal/numberofequilibrium/dble(Npart)
      642         format(1x,'CYCLES:',I8, '  AveEnergy:',E15.6)
                  write(62,644)Npart,totAcceptanceRatio/dble(iEqui),deltaRx
                  write(51,644)Npart,totAcceptanceRatio/dble(iEqui),deltaRx
...
!       --------------
        ...
        IF(SDisplace .AND. (.NOT. TradMC))THEN
            ...
            do iCycle = 1, Ncycles1
               ...
               if (mod(iCycle,1000).eq.0)then
                  write(62,*)'=============Mu-NVT SAMPLING==================='
                  write(51,*)'=============Mu-NVT SAMPLING==================='
                  if(ExtraBin)then
                     write(62,711)iCycle, (EnergyTotal+ExtEnergyTotal)/numberOfSampling/dble(Npart)
                     write(51,711)iCycle, (EnergyTotal+ExtEnergyTotal)/numberOfSampling/dble(Npart)
      711            format(1x,'CYCLES:',I8, '  AveEnergy:',E15.6)
                  else
                      write(62,711)iCycle, EnergyTotal/numberOfSampling/dble(Npart)
                      write(51,711)iCycle, EnergyTotal/numberOfSampling/dble(Npart)
                  endif
                  write(62,712)Npart,totAcceptanceRatio/dble(iCycle),deltaRx
                  write(51,712)Npart,totAcceptanceRatio/dble(iCycle),deltaRx
        ...
        ELSEIF(TradMC)THEN
          ...
          do iCycle=1, NCycles
              ...
              if( mod(iCycle,1000).eq.0) then
                 ...
                 write(62,*)'==================NVT SAMPLING====================='
                 write(51,*)'==================NVT SAMPLING====================='
                 write(62,715)iCycle, EnergyTotal/numberOfSampling
                 write(51,715)iCycle, EnergyTotal/numberOfSampling
      715        format(1x,'CYCLES:',I8,'  AveEnergy:',e15.6)
                 write(62,718)Npart,totAcceptanceRatio/dble(icycle),deltaRx
                 write(51,718)Npart,totAcceptanceRatio/dble(icycle),deltaRx
  ...
  !          arg1 = dble(Npart)*debro3*exp((TEnergyOld-Chemicalpotential(iPC))/Temperature)/BulkVolume
            ...
            if(rdn .lt. arg)then     ! DELETE   !delete if-2
               ...
               IF(ADSORPTION.AND. LOCALF)THEN    !delete if-3
                  ...
                  !write(62,*)'line 1633 negative NinBin,bin',NinBin(bin),bin
                  !read(62,*)
                  endif
                  do j = 1,Npart-1
                     if(j.NE.old)then
...
!                EnergyMatrix(old,old) = EnergyMatrix(Npart,Npart)     
!                Npart    = Npart - 1
!                  WRITE(62,*)'1595 after DELETE Energy,EnergyOld,Npart',Energy,EnergyOld,Npart
!                   write(62,*)(Energymatrix(1:npart, 1:npart))
!                  read(62,*)
...
!$OMP private (clvectorz,clvectorz0,d2,energycij,gsm,ljvectorx)   &
!$OMP private (ljvectorx0,ljvectory,ljvectory0,ljvectorz,ljvectorz0)  &
!$OMP private (mcd2,mcvectorx,mcvectorx0,mcvectory,mcvectory0)      &
...
!$OMP shared  (localf,ncl,npart,nlj,r2cutoff,smediation1) &
!$OMP shared  (smediation2,smlimitz,subbox,temperature) &
...
!                  WRITE(62,*)'2585 after move Energy,npart',Energy,npart
                   IF(Adsorption.AND. LOCALF)THEN 
                       bin0 = I_POS(i) 
...
!       ------------------
        numberOfMove           = 0
        numberOfAcceptanceMove = 0
!        write(62,*)'AcceptanceRatio,deltar',AcceptanceRatio,deltar
        return
        end 
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++      
        ...
        write(62,*)'The Calculation of the density is in processing'
 ...
 !               WRITE(62,*)'rho1,P1,Pressure(iP)',rho1,P1,Pressure(iP)
 !               read(62,*)
                F1 = P1 - Pressure(iP)
                rho2= rho1+delta_rho
                IF(JOHNSON)call JohnsonPCalculation( rho2, P2 )
                IF(WAGNER)call WagnerPCalculation( rho2, P2 )
 !               WRITE(62,*)'rho2,P2,Pressure(iP)',rho2,P2,Pressure(iP)
 !               read(62,*)
                F2 = P2 - Pressure(iP)
...
!            write(62,*)j
!            do i=1, j
!            write(62,*)'i, rhos(i)',i, rhos(i)
!            write(51,*)'i, rhos(i)',i, rhos(i)
!            enddo

            rho(iP) = rhos(1)
            rhoMolPerM3(iP) = rho(iP)/(SCALELENGTH*1.0E-10)**3/AvogadroNumber   
          ENDIF
         write(51,*)'iP, rho(-&mol/m3)',iP, rho(iP), rhoMolPerM3(iP)
      enddo
...
!    ------------------------------------------------------
     ...
     if(rcutoff.gt.rcutoffset)then
       write(62,*)'------------------------------------- '
       write(62,*)'Which one would choose as the rcutoff:'
       write(62,*)'------------------------------------- '
       write(62,*)'1-', rcutoff
...
!    --------------------------------
     ...
     write(62,*) 'BULKPHASE GCMC'
     write(51,*) 'BULKPHASE GCMC'
...
1130 format( 1x, 'SurfaceArea(-)--------------------------:', f20.10,/, &
  &          1x, 'BulkVolume(-)---------------------------:', f20.10,/)  
    ...
    do iPC = 1 , NumOfPre
       write(62,*)'---------------------------------------------------------------------------'
       write(62,*)'BULKPHASE The pressure being calculated is _ of _ - _:', iPC,NumOfPre,P(iPC)
       write(62,*)'---------------------------------------------------------------------------'    
!      -------------------------
!      SET THE DISPLACEMENT STEP 
!      -------------------------
...
!    --------------------------------
     ...
     do iEqui = 1, Ncycle
         if (mod(iEqui,1000).eq.0)then
            write(62,*)'================BULK EQULIBRATION==================='
            write(51,*)'================BULK EQULIBRATION==================='
            write(62,645)iEqui, sum_Npart/numberofequilibrium, EnergyTotal/numberofequilibrium
            write(51,645)iEqui, sum_Npart/numberofequilibrium, EnergyTotal/numberofequilibrium
...
!    -----------------------------
     ...
     do iCycle=1, NCycles
         ...
         if( mod(iCycle,1000).eq.0) then
            ...
            write(62,*)'==================BULK SAMPLING====================='
            write(51,*)'==================BULK SAMPLING====================='
            write(62,716)iCycle, sum_Npart/numberOfSampling, EnergyTotal/numberOfSampling
            write(51,716)iCycle, sum_Npart/numberOfSampling, EnergyTotal/numberOfSampling
...
!              write(62,*)'distanceZ,reducedX,Fun_pentagamma,term1,term2,iEnergy',distanceZ,reducedX,Fun_pentagamma,term1,term2,iEnergy
!              write(62,*)'eta,bsf_b,alphasf_b,alphaSF_B*distanceZ,exp(alphaSF_B*distanceZ)',eta,bsf_b,alphasf_b,alphaSF_B*distanceZ,exp(alphaSF_B*distanceZ)
!              read(62,*)
!              endif
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++               
 ...
 !      sum_UFFNout1toBin  = 0.0e0
     ...
     if(switch.eq.2)then
        ...
        sumlocalEffj = 0.0e0
        sumlocalEsf = 0.0e0
!         write(62,*)'fluctuation 2 CountN',CountN        
 ...
 !         sum_UFFNout1toBin(i)  = sum_UFFNout1toBin(i) + UFFout1toBin(i)*N1toBin(i)
        ...
        enddo
!        write(62,*)'i,UFF1toBin(i),UFFout1toBin(i),USF1toBin(i)',maxbin, UFF1toBin(maxbin),UFFout1toBin(maxbin),USF1toBin(maxbin) 
!        read(62,*)    
!         write(62,*)'sumlocalE,sumlocalEffi,sumlocalEffj,sumlocalEsf', sumlocalE,sumlocalEffi,sumlocalEffj,sumlocalEsf
!         read(62,*)
     endif
